...
img_color = cv2.imread('.vscode\score2.png', cv2.IMREAD_COLOR)
img_original = cv2.cvtColor(img_color, cv2.COLOR_BGR2GRAY)

img_template = cv2.imread('.vscode\mat5.png', cv2.IMREAD_GRAYSCALE)
img_resize = cv2.resize(img_template, (18,18), interpolation=cv2.INTER_CUBIC)
original_h, original_w = img_original.shape[:2]
#template_h, template_w = img_template.shape[:2]
template_h, template_w = img_resize.shape[:2]
...
for original_x in range(original_w - template_w):
    for original_y in range(original_h - template_h):
        ...
        template_pixel = img_resize.ravel()
...
for p in point:
    if np.abs(p[2] - best_position_sad) < 100:
        print(p[0],p[1])
        print(p[0] + template_w)
        print(p[1] + template_h)
        print(p[2])
        cv2.rectangle(img_color, (p[0], p[1]), (p[0] + template_w, p[1] + template_h), (255, 0, 0), 3)

cv2.imshow('result', img_color)
cv2.imwrite(".vscode\mattest.png",img_color)
cv2.waitKey(0)